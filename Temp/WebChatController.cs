                ...
                else
                {
                    ...
                    if (File != null)
                    {
                        ...
                        while (remaining > 0)
                        {
                            ...
                            {
                                read = File.InputStream.Read(buffer, 0, buffer.Length);
                                JefferiesExcuReport.SendFile(File.FileName, buffer, sequence, totalSequence, sender);
                            }
                            else if (sequence == totalSequence && remaining % buffer.Length > 0)
                            {
                                read = File.InputStream.Read(lstBuffer, 0, lstBuffer.Length);
                                JefferiesExcuReport.SendFile(File.FileName, lstBuffer, sequence, totalSequence, sender);
                            }
                            remaining -= read;
                            sequence++;
                        }
