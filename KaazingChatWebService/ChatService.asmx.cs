...
using KaazingChatWebService.Classes;
...
namespace KaazingChatWebService
{
    ...
    public class ChatService : System.Web.Services.WebService
    {
        ...
        [WebMethod]
        public bool SendMessageToClient1(List<string> WebServiceFixMessageList, string sendTopic)
        {
            //if (Debugger.IsAttached == false)
            //    Debugger.Launch();
            if (WebServiceFixMessageList.Count == 0)
            {
                return false;
            }
            ...
            try
            {
                ...
                foreach (string WebServiceFixMessage in WebServiceFixMessageList)
                {
                    string sfixMessage = FixStringBuilder.ConvertToGenericFixString(WebServiceFixMessage);
                    JefferiesExcuSendTag JefferiesExcuSendTag = FixStringBuilder.ConvertToType<JefferiesExcuSendTag>(sfixMessage);
                    FixStringBuilder.AddTypeToList<JefferiesExcuSendTag>(JefferiesExcuSendTag, ref JefferiesExcuSendTagList);
                }

                string sJson = JsonConvert.SerializeObject(JefferiesExcuSendTagList);
                JefferiesExcuReport.Start();
                JefferiesExcuReport.SendMessage(sJson);
                SendMessageResult = true;
            }
            catch (Exception ex)
            {
        ...
        [WebMethod]
        public bool SendMessageToClient2(string sJson, string sendTopic)
        {
            //if (Debugger.IsAttached == false)
            //    Debugger.Launch();
            //test code begin
